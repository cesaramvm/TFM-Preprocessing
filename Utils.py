...
def removeBordersByPercentage(image, percentage=REMOVE_BORDERS_PERCENTAGE):
    ...
    return (image[cropPixelsHeight:height-cropPixelsHeight, cropPixelsWidth:width-cropPixelsWidth],cropPixelsWidth,cropPixelsHeight)
...
def checkCntTouchesCorners(cnt, imgWidth, imgHeight):
    ...
    if touchesCorner:
        ...
        M = cv2.moments(cnt)
        if (M["m00"] == 0):
            return True #No es que toque bordes pero área 0...muy feo.
        cX = int(M["m10"] / M["m00"])
        lateralPositioned = cX < lateralContourDoubleCheckPixelWidth or cX > imgWidth-lateralContourDoubleCheckPixelWidth
        # lateralPositioned = True
    return touchesCorner and lateralPositioned


def getCntsInfo(cnts, imgWidth, imgHeight, blur1, removedBordersImage, fileNameFull, showContours = SHOW_CONTOURS):
